<?php
  ...
  $ROOTDIR = 'http://localhost/blog/build/public/' . $detailurl; // todo..后期上线需要修改
  // 注意!!开发期间，这里的拷贝需要同时拷贝到 src ，不然编译得到的 build/dist 会丢失图片
  $devOnlyDir = "../../../src/" . $detailurl;
  if (!file_exists($devOnlyDir)) {
    // echo 'is going to mkdir..$devOnlyDir : ' . $devOnlyDir;
    mkdir($devOnlyDir, 0777, true); // 第三个参数表示递归
    chmod($devOnlyDir, 0777);
  }
  ...
  foreach($_FILES as $img){
    ...
    $devOnlyImgDir = $devOnlyDir . '/' . $img['name']; // 仅开发需要
    // 数据库操作需要的字段
    ...
    if(file_exists($img_dir)){
      // 替换服务器上原有图片
      unlink($img_dir);
      unlink($devOnlyImgDir); // 上线的话需要修改
      if(move_uploaded_file($img['tmp_name'], $devOnlyImgDir)){ // move_uploaded_file 上线的话需要修改
         // && move_uploaded_file($img['tmp_name'], $img_dir) // 开发时，只能对 src 中的进行修改？OAO
      ...
      // 上传图片到服务器
      if(copy($img['tmp_name'], $devOnlyImgDir)
         && copy($img['tmp_name'], $img_dir)){ // copy 上线的话需要修改
        $insertFlag = true;
      } else {
        array_push($response, array(
